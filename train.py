...
class TrainClassifier:
    ...
    def train(self):
        ...
        """
        # Loading the Dataset
        trainset, validset, testset, num_cls = load_dataset_custom(self.configdata['dataset']['datadir'], self.configdata['dataset']['name'], self.configdata['dataset']['feature'])
        N = len(trainset)
        trn_batch_size = 20
        val_batch_size = 1000
        tst_batch_size = 1000
        ...
        if self.configdata['dss_strategy']['type'] == 'GradMatch':
            ...
            if 'kappa' in self.configdata['dss_strategy']:
                ...
                full_epochs = round(kappa_epochs * int(self.configdata['dss_strategy']['fraction'] * 100))
            else:
