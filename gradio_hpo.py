...
def hpo(dset, ml_model, strategy, budget):
        ...
        for search_config in search_configs:
            ...
            if dset in ['trec6']:
                ...
                if "learning_rate" in search_config:
                    config_data.optimizer.lr = search_config["learning_rate"]    
            else:
    ...
    # fig, ax = plt.subplots()
    hpo_dict = {key1:[], key2:[], key3:[], "Validation Accuracy using selected strategy": [], "Full training Validation Accuracy": []}
    best_strat_acc = -1
    best_full_acc = -1
    strat_tuning_time = 0
    full_tuning_time = 0
    ...
    for i in range(len(search_configs)):
        ...
        hpo_dict[key3].append(search_configs[i][key3])
...
with gr.Blocks(title = "Hyper-parameter Optimization") as demo:
    with gr.Row():
        with gr.Column():
            ...
            strategy = gr.Dropdown(choices=['Random', 'AdaptiveRandom', 'MILO', 'WRE', 'SGE', 'MILOFixed', 'GradMatchPB', 'CraigPB', 'GLISTER', 'FULL'], label='Subset Selection Strategy')
